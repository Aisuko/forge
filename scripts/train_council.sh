...
#
#   ./scripts/train_council.sh              # everything
#   ./scripts/train_council.sh --branch     # experts only (ancestor already trained)
#
...
mkdir -p "$DIR" logs
[[ -f data/council/manifest.json ]] || ./scripts/split_corpus.py
...
echo
echo "done. next: cargo run --release --example council_demo"
